...
class WIREVIS_PT_scene(Panel):
    ...
    def draw(self, context):
        ...
        if not context.scene.wv_params.wv_display:
            row.operator("view3d.wirevis2", text="Display")
        else:
...
class VIEW3D_OT_WireVis2(Operator):
    bl_idname = "view3d.wirevis2"
    bl_label = "Wire Vis"
        ...
        if wvp.update:
            ...
            for oi, ob in enumerate(wv_obs):
                ...
                for ei, ec in enumerate(ecentres):
                    ...
                    # mat_rot = mathutils.Vector((0, 0, 1)).rotation_difference([v.co for v in edge.verts][1] - ec).to_matrix().to_4x4()
...
classes = (WIREVIS_PT_object, WIREVIS_Object_Settings, WIREVIS_Scene_Settings, VIEW3D_OT_WireVis, VIEW3D_OT_WireVis2, WIREVIS_PT_scene, VIEW3D_OT_WireCancel)
